...
export function buildRows(config: PolycodeConfig): Row[] {
  ...
  for (const key of Object.keys(config.agents)) {
    ...
    rows.push({ kind: 'field', path: `agents.${key}.description`, label: `${key}.description`, type: 'string' });
    rows.push({ kind: 'field', path: `agents.${key}.cmdTemplate`, label: `${key}.cmdTemplate`, type: 'string' });
  ...
  config.tiers.forEach((t, i) => {
    ...
    rows.push({ kind: 'field', path: `tiers.${i}.keywords`, label: `tier ${i} keywords (csv)`, type: 'csv' });
  ...
  for (const key of Object.keys(config.swarms)) {
    ...
    if (swarm.type === 'sequential') {
      swarm.stages.forEach((st, i) => {
        rows.push({ kind: 'field', path: `swarms.${key}.stages.${i}.name`, label: `${key}.stage${i}.name`, type: 'string' });
        rows.push({ kind: 'field', path: `swarms.${key}.stages.${i}.agentKey`, label: `${key}.stage${i}.agent`, type: 'string' });
        rows.push({ kind: 'field', path: `swarms.${key}.stages.${i}.cmd`, label: `${key}.stage${i}.cmd`, type: 'string' });
      ...
      swarm.subagents.forEach((sb, i) => {
        rows.push({ kind: 'field', path: `swarms.${key}.subagents.${i}.name`, label: `${key}.sub${i}.name`, type: 'string' });
        rows.push({ kind: 'field', path: `swarms.${key}.subagents.${i}.agentKey`, label: `${key}.sub${i}.agent`, type: 'string' });
        rows.push({ kind: 'field', path: `swarms.${key}.subagents.${i}.taskTemplate`, label: `${key}.sub${i}.taskTemplate`, type: 'string' });
...
function applyAction(config: PolycodeConfig, op: string): { config: PolycodeConfig; note?: string } {
  ...
  switch (verb) {
    case 'add-agent': {
      ...
      return {
        config: setPath(config, `agents.${key}`, { name: key, costBadge: '', description: '', cmdTemplate: 'echo "{prompt}"' }),
        note: `added agent "${key}"`,
      };
    }
    case 'del-agent': {
      const refs = agentRefs(config, arg1);
      if (refs.length) return { config, note: `cannot delete "${arg1}" — referenced by ${refs.join(', ')}` };
      const agents = { ...config.agents };
      delete agents[arg1];
      return { config: { ...config, agents }, note: `deleted agent "${arg1}"` };
    }
    case 'add-tier': {
      const t = { id: uniqueKey(config.tiers.map((x) => x.id), 'new-tier'), name: 'New Tier', color: 'white', costBadge: '', agentKey: Object.keys(config.agents)[0] ?? '', maxWords: null, keywords: [] as string[] };
      return { config: { ...config, tiers: [...config.tiers, t] }, note: `added tier "${t.id}"` };
    }
    case 'del-tier': {
      const i = Number(arg1);
      return { config: { ...config, tiers: config.tiers.filter((_, j) => j !== i) }, note: `deleted tier ${i}` };
    }
    case 'add-swarm': {
      const key = uniqueKey(Object.keys(config.swarms), 'new-swarm');
      const firstAgent = Object.keys(config.agents)[0] ?? '';
      return {
        config: setPath(config, `swarms.${key}`, { name: key, type: 'parallel', subagents: [{ id: 'sub-1', agentKey: firstAgent, name: 'Subagent 1', taskTemplate: '{prompt}' }] }),
    ...
    case 'add-stage': {
      ...
      const firstAgent = Object.keys(config.agents)[0] ?? '';
      const stages = [...swarm.stages, { agentKey: firstAgent, name: `Stage ${swarm.stages.length + 1}`, cmd: 'echo "{prompt}"' }];
      return { config: setPath(config, `swarms.${arg1}.stages`, stages), note: `added stage to "${arg1}"` };
    }
    case 'del-stage': {
      const swarm = config.swarms[arg1];
      if (!swarm || swarm.type !== 'sequential') return { config };
      return { config: setPath(config, `swarms.${arg1}.stages`, swarm.stages.filter((_, j) => j !== Number(arg2))), note: `deleted stage ${arg2} of "${arg1}"` };
    }
    case 'add-sub': {
      const swarm = config.swarms[arg1];
      if (!swarm || swarm.type !== 'parallel') return { config, note: 'not a parallel swarm' };
      const firstAgent = Object.keys(config.agents)[0] ?? '';
      const subs = [...swarm.subagents, { id: uniqueKey(swarm.subagents.map((s) => s.id), `sub-${swarm.subagents.length + 1}`), agentKey: firstAgent, name: `Subagent ${swarm.subagents.length + 1}`, taskTemplate: '{prompt}' }];
      return { config: setPath(config, `swarms.${arg1}.subagents`, subs), note: `added subagent to "${arg1}"` };
    }
    case 'del-sub': {
      const swarm = config.swarms[arg1];
      if (!swarm || swarm.type !== 'parallel') return { config };
      return { config: setPath(config, `swarms.${arg1}.subagents`, swarm.subagents.filter((_, j) => j !== Number(arg2))), note: `deleted sub ${arg2} of "${arg1}"` };
    }
    default:
      return { config };
  }
...
function coerce(type: FieldType, raw: string): { ok: true; value: unknown } | { ok: false; why: string } {
  switch (type) {
    ...
    case 'bool': return { ok: true, value: raw === 'true' };
    case 'csv': return { ok: true, value: raw.split(',').map((s) => s.trim()).filter(Boolean) };
  }
}
...
export function ConfigEditor({ config, onClose, onSave }: Props) {
  ...
  const height = Math.max(8, (stdout?.rows ?? 24) - 8);
  ...
  function beginEdit(idx: number) {
    ...
    if (row.kind === 'action') {
      const res = applyAction(draft, row.op);
      setDraft(res.config);
      setNote(res.note ?? row.op);
      return;
    }
    if (row.type === 'bool') {
  ...
  // Own keyboard handling — App's useInput is inactive while this is open.
  useInput(
    (input, key) => {
      if (editing) {
      ...
      if (input === 'k') { move(-1); return; }
      if (input === 'j') { move(1); return; }
      if (key.tab) { move(key.shift ? -5 : 5); return; }
      if (key.return) { beginEdit(cursor); }
    },
    { isActive: true },
  );
    ...
    <Box flexDirection="column">
      <Box>
        <Text bold color={dirty ? 'yellow' : undefined}>
          config editor{dirty ? ' (unsaved)' : ''}
        </Text>
        <Text dimColor> · j/k move · enter edit · a/d add/del · ctrl+s save · esc close</Text>
      </Box>
      <Box flexDirection="column" height={height}>
        {visible.map((row, vi) => {
          ...
          if (row.kind === 'section') {
            return (
              <Box key={`s:${row.label}`} marginTop={idx === 0 ? 0 : 1} paddingLeft={2}>
          ...
          if (row.kind === 'action') {
            return (
              <Box key={`a:${row.op}`} paddingLeft={4}>
          ...
          return (
            <Box key={`f:${row.path}`} paddingLeft={4}>
              <Text color={active ? 'cyan' : 'gray'}>{active ? '> ' : '  '}</Text>
              <Box width={Math.min(30, Math.floor(width * 0.35))} flexShrink={0}>
              ...
              {isEditing ? (
                ...
                <Text
                  color={
                    row.type === 'bool'
                      ? value === 'true' ? 'green' : 'red'
                      : value === '' || value === '(null)' ? 'gray' : undefined
                  }
                >
                  {row.type === 'bool'
                    ? value === 'true' ? 'yes' : 'no'
                    : value === '' ? '(empty)' : truncate(value, Math.max(10, width - 40))}
                </Text>
              )}
            </Box>
          );
      ...
      <Box paddingLeft={2}>
        <Text dimColor>
          ...
          {dirty ? ' · ctrl+s to write' : ''}
        </Text>
        {note ? <Text color="yellow"> · {truncate(note, 60)}</Text> : null}
      </Box>
    </Box>
  );
}